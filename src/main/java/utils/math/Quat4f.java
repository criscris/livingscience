...
public class Quat4f extends Tuple4f implements java.io.Serializable {
  ...
  static Matrix3f identityRot;
  public final void setIdentity()
  {
	  if (identityRot == null)
	  {
		  identityRot = new Matrix3f();
		  identityRot.setIdentity();
	  }
	  
	  set(identityRot);
  }
  
  public static final Quat4f identity()
  {
	  Quat4f q = new Quat4f();
	  q.setIdentity();
	  return q;
 ...
 /**
   ...
   */
  public final void fillEuler(Vector3f euler)
  {
	  euler.x = (float) Math.atan2(2f * (x*y + z*w), 1f - 2f * (y*y + z*z));
	  euler.y = (float) Math.asin(2f * (x*z - w*y));
	  euler.z = (float) Math.atan2(2f * (x*w + y*z), 1f - 2f * (z*z + w*w));
  }
   ...
   * from http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
   */
  public final void setFromEuler(Vector3f euler)
  {
