...
public class Matrix3f implements java.io.Serializable {
  ...
  /** 
   ...
   /** 
	 ...
	 */
    private static final Matrix3f rotPerp = new Matrix3f();
	public final void setRotation(Vector3f source, Vector3f target)
	{
		float dot = source.dot(target);
		dot = Math.min(1f, dot);
		
//		System.out.println("dot " + source + " � " + target + " = " + dot);
		...
		if (Math.abs(Math.abs(angle) - Math3D.PI) < 0.001)
		{
			//...
			System.out.println("angle 180");
			
			rotPerp.rotX(Math3D.PI/2f);
			source = new Vector3f(source);
			rotPerp.transform(source);
			
			rotPerp.rotY(Math3D.PI/2f);
			source = new Vector3f(source);
			rotPerp.transform(source);
		}
...
//    }
	...
	public final void setToOuterProduct(Vector3f column, Vector3f row)
	{
		m00 = column.x * row.x;
		m01 = column.x * row.y;
		m02 = column.x * row.z;
	...
	public final boolean hasNaNorInf()
	{
		return Float.isNaN(m00) || Float.isInfinite(m00) ||
				Float.isNaN(m01) || Float.isInfinite(m01) ||
				Float.isNaN(m02) || Float.isInfinite(m02) ||
				Float.isNaN(m10) || Float.isInfinite(m10) ||
				Float.isNaN(m11) || Float.isInfinite(m11) ||
				Float.isNaN(m12) || Float.isInfinite(m12) ||
				Float.isNaN(m20) || Float.isInfinite(m20) ||
				Float.isNaN(m21) || Float.isInfinite(m21) ||
				Float.isNaN(m22) || Float.isInfinite(m22);
	}
}
